...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'&gt;</xsl:text>
            ...
            <xsl:for-each select="//text:footnote">
                <p class="footnote">
                    <span>
                        <xsl:element name="a">
                            <xsl:attribute name="href">
                                <xsl:value-of select="concat('#body', @text:id)"/>
                            </xsl:attribute>
    ...
    <xsl:template match="style:header">
        <xsl:comment>Removed document header:
            <xsl:value-of select="."/>
        </xsl:comment>
    </xsl:template>

    <xsl:template match="style:footer">
        <xsl:comment>Removed document footer:
            <xsl:value-of select="."/>
        </xsl:comment>
    </xsl:template>
    ...
    * Enter the ALT text to be used in the "Alternative (Text Only)" field.
    ...
    <xsl:template match="//draw:image">
        <xsl:text disable-output-escaping="yes">&lt;img alt="</xsl:text><xsl:value-of select="svg:desc"/><xsl:text
            disable-output-escaping="yes">" src="images/</xsl:text><xsl:value-of select="@draw:name"/><xsl:text
